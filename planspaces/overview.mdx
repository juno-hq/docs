...
- approved intent that should later be pulled into execution with `scott pull`

## How planspaces start

You can create a planspace three ways:

1. **From Sync Chat** - pin or write up a useful synced session after `scott sync`
2. **From your coding agent** - run [`scott push`](/developer-tools/push) from a focused thread
3. **From the app** - start a new planspace and describe what you want Scott to draft

For daily team workflows, `scott push` is usually the fastest path. It turns active agent work into a shareable document while the idea is still fresh.
...
2. Push it into a planspace with [`scott push`](/developer-tools/push).
...
6. Pull the reviewed context into a fresh agent thread with [`scott pull`](/developer-tools/pull).
