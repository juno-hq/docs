...
`scott pull` fetches the current planspace context into your terminal or agent thread. When you are ready to ship, include the planspace URL in your PR description or a commit message so [Scott CI](/developer-tools/scott-ci) can trace the implementation back through the context flow to the [approved spec](/collaboration/reviews).
...
# Add the planspace URL to the PR description so Scott CI can link it.
```
