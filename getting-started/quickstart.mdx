...
Sync is the fastest way to onboard. Open Claude Code, Codex, Cursor, OpenCode, Gemini, or another supported coding agent on your development machine and paste:

```text
Install Scott AI and run sync so Scott can understand my recent local agent work.
```

The Scott plugin installs the needed skill and command support, reads recent local coding-agent sessions, and opens `/sync`. After sync finishes, open the URL Scott prints and ask across recent Claude Code, Codex, Cursor, OpenCode, Gemini, and other supported agent sessions.

<Tip>
  Power users can also run sync directly from a local terminal with `curl -fsSL https://tryscott.ai/install | sh` and `scott sync`. Agent invocation is the default path, and push/pull should stay agent-invoked because they depend on the active thread context.
</Tip>
