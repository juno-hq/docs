...
`scott sync` is the fastest way to onboard. It uploads recent local coding-agent sessions, opens the Sync Chat view, and starts an analysis of what Scott can now see.
...
After sync finishes, open the URL printed by the CLI. Scott will take you to **For You > Sync**, where you can ask across recent Claude Code, Codex, Cursor, OpenCode, Gemini, and other supported agent sessions.

<Tip>
  Run `scott sync` from your development machine, not from a hosted browser agent. The command reads local agent history before uploading relevant context to Scott.
</Tip>
...
Use `scott push` whenever an agent thread becomes a prototype, design direction, or proposal that needs team feedback.

```bash
scott push
```

Scott creates a planspace with a readable document and a URL you can share in Slack, Linear, a design review, or a PR description. Teammates can comment, request changes, and approve a specific document version.

When the team is aligned, pull the planspace into a fresh agent thread:
...
<CardGroup cols={2}>
  ...
  <Card title="Scott GitHub App" icon="github" href="/developer-tools/github-app">
    Learn how repository context and PR design review connect planspaces to code.
  </Card>
